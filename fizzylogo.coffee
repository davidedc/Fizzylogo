...
class RosettaObjects
            ...
            if eachElementOfSignature.rosettaClass == RAtom
              ...
              if isEvaluatingParam(eachElementOfSignature)
                ...
                if method.firstElement().rosettaClass == RList
                  ...
                  [valueToBeBound, method] = rosettaEval method, theContext
              else
  ...
  messageSend: (methodBody, theContext) ->
    ...
    if methodBody.rosettaClass == RList
      ...
      [theContext, unusedRestOfMessage] = @nonNativeMessageSend methodBody, theContext
    else
  ...
  nonNativeMessageSend: (message, theContext) ->
    ...
    newContext = new RosettaContext theContext, @, message
...
rosettaEval = (tokenOrList, theContext) ->
  console.log "           " + indentation() + "evaling: " + tokenOrList.print()
  [newContext, unusedRestOfMessage] = tokenOrList.progressWithMessage emptyMessage(), theContext
  if tokenOrList.rosettaClass == RList
    tokenOrList = tokenOrList.advanceMessageBy newContext.programCounter
  return [newContext.returned, tokenOrList]
